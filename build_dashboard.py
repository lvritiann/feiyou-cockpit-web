...
function renderForecast(){
  ...
  const k = fc.kpis || {};
  const ex = fc.extra || {};
  // KPI —— 主数字 = 全年预计；副标题 = 已完成的实际数（截至 asOf）
  ...
  const remPred = (k['剩余预测万']!=null) ? Number(k['剩余预测万'])
              : ((S0!=null && natCum!=null && k['华山扣除万']!=null) ? S0-Number(natCum)+Number(k['华山扣除万']) : null);
  ...
  const cards = [
    {k:'预计全年 · 自然口径', v:N(S0), u:'万',
     s:(remPred!=null && k['华山扣除万']!=null)
        ? `实际累计 ${N(natCum)} 万 ＋ 剩余预测 ${N(remPred)} 万 − 华山扣除 ${N(k['华山扣除万'])} 万`
        : `实际累计 ${N(natCum)} 万`},
    {k:'预计任务达成率 · 自然口径', v:(ratePlan==null?'—':ratePlan.toFixed(2)), u:'%',
     warn:(ratePlan!=null && ratePlan<100),
     s:`实际进度 ${rateReal==null?'—':rateReal.toFixed(2)}% · 距 ${task} 万任务还差 ${N(S0==null?null:task-S0)} 万`},
  ...
  document.getElementById('fcKpis').innerHTML = cards.map(c=>
    ...
    + `<div class="note" style="grid-column:1/-1;margin-top:0">卡片<b>主数字 = 全年预计</b>，副标题 = 已完成的实际数（截至 ${fc.asOf||'—'}）。达成率按<b>自然经营口径</b>（不含烟草批发去化）对全年 ${task} 万任务；含去化的全口径超额率见下方「销量任务超额」图。</div>`;
